...
const StickyNav: React.FC<{ activeSection: string; }> = ({ activeSection }) => {
    ...
    const ns = 'services_pages.dewatering';
    const navItems = [
        { id: 'overview', label: t(`${ns}.nav_overview`) },
        { id: 'methods', label: t(`${ns}.nav_methods`) },
...
const DewateringServices: React.FC = () => {
    const { t } = useTranslation();
    const ns = 'services_pages.dewatering';
    ...
    return (
        <>
            ...
            <section id="overview" ref={sectionRefs.overview} className="py-12 md:py-20 bg-jis-bg-gray relative overflow-hidden">
                <div className="absolute inset-0 dewatering-pattern-light opacity-30"></div>
            ...
            <section id="methods" ref={sectionRefs.methods} className="py-12 md:py-20 bg-white relative overflow-hidden">
                <div className="absolute inset-0 dewatering-pattern-light opacity-10"></div>
                <div className="container relative mx-auto px-4 sm:px-6 lg:px-8 space-y-12 md:space-y-20">
                    <div className="flex flex-col items-center gap-8 md:flex-row lg:gap-12">
                        <div className="w-full flex-1">
                            <h3 className="text-2xl font-bold leading-tight text-slate-900">{t(`${ns}.method1_title`)}</h3>
                            <p className="mt-2 text-base font-normal leading-normal text-slate-600">{t(`${ns}.method1_desc`)}</p>
                        </div>
                        <div className="aspect-video w-full flex-1 rounded-lg bg-cover bg-center shadow-md" style={{backgroundImage: `url('${t(`${ns}.method1_image`)}')`}}></div>
                    </div>
                    <div className="flex flex-col items-center gap-8 md:flex-row-reverse lg:gap-12">
                        <div className="w-full flex-1">
                             <h3 className="text-2xl font-bold leading-tight text-slate-900">{t(`${ns}.method2_title`)}</h3>
                            <p className="mt-2 text-base font-normal leading-normal text-slate-600">{t(`${ns}.method2_desc`)}</p>
                        </div>
                        <div className="aspect-video w-full flex-1 rounded-lg bg-cover bg-center shadow-md" style={{backgroundImage: `url('${t(`${ns}.method2_image`)}')`}}></div>
                    </div>
                </div>
            </section>
...
export default DewateringServices;