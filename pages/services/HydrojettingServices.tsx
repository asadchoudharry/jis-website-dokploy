...
const TechStep: React.FC<{ step: string; title: string; description: string }> = ({ step, title, description }) => (
...
const HydrojettingServices: React.FC = () => {
    const { t } = useTranslation();
    const ns = 'services_pages.hydrojetting';
    ...
    const techStepsData = t(`${ns}.tech_steps`, { returnObjects: true });
    const techSteps = Array.isArray(techStepsData) ? techStepsData : [];
    ...
    return (
        <>
            ...
            <section id="overview" ref={sectionRefs.overview} className="py-12 md:py-20 bg-jis-bg-gray relative overflow-hidden">
                <div className="absolute inset-0 hydro-pattern-light opacity-50"></div>
            ...
            <section id="applications" ref={sectionRefs.applications} className="py-12 md:py-20 bg-white relative overflow-hidden">
                <div className="absolute inset-0 hydro-pattern-light opacity-25"></div>
            ...
            <section id="technology" ref={sectionRefs.technology} className="py-12 md:py-20 bg-jis-bg-gray relative overflow-hidden">
                <div className="absolute inset-0 hydro-pattern-light opacity-50"></div>
                <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center max-w-3xl mx-auto mb-10">
                        <h2 className="text-3xl font-bold text-slate-900">{t(`${ns}.tech_title`)}</h2>
                        <p className="mt-4 text-lg text-slate-600">{t(`${ns}.tech_subtitle`)}</p>
                    </div>
                    <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4 max-w-7xl mx-auto">
                        {techSteps.map(step => (
                            <TechStep key={step.step} step={step.step} title={step.title} description={step.description} />
                        ))}
                    </div>
                </div>
            </section>
...
export default HydrojettingServices;